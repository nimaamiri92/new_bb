...
class CategoryController extends BaseController
{
    ...
    public function store(CreateCategoryRequest $request)
    {
        ...
        if ($category === 0) {
            return redirect()->route('admin.categories.create')->with('message', 'حداکثر تعداد منو/دسته بندی اصلی ۵ عدد می باشر');
        }
    ...
    public function edit(Category $category)
    {
        ...
        $this->setSideBar('categories');
        $categories = $this->categoryRepository->all();
        return view('admin.categories.edit', compact('category', 'categories', 'category'));
    }
    ...
    public function uploadImages(UploadImagesRequest $request, Category $category)
    {
        ...
        $this->categoryRepository->saveImages(collect($data), $category);
